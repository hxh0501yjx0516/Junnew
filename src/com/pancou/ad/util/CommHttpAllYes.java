...
public class CommHttpAllYes  {
	...
	public synchronized Properties send(String strUrl) {
		...
		try {
			...
			if (result.indexOf("result:true") != -1 || result.indexOf("ok") != -1) {
				prop.put("result", "0");
				prop.put("desc", "成功");
			} else {
				prop.put("result", "1");
			...
			//log.info(strurl + "\tresp: " + result+"\t"+(endtime-starttime)+" ms");

		} catch (Exception e) {
			endtime = System.currentTimeMillis();
			//log.error(strurl + "\tresp: " + e.getMessage()+"\t"+(endtime-starttime)+" ms");
			e.printStackTrace();
			return null;
		} finally {
			try {
